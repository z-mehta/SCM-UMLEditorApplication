...
public static boolean enable, enable1, enable2;
   public static List<INode> alNodes = new ArrayList<INode>();
    /**
...
if(enable){
    ...
    if( (this.getClass().toString().endsWith("ClassDiagramGraph") && (e.getClass().toString().endsWith("AggregationEdge")
            || e.getClass().toString().endsWith("CompositionEdge"))) ) {

                    if ( (e.getStartNode() == e.getEndNode()) && (Collections.frequency(alNodes,e.getStartNode())) > 1 ) {
                            JOptionPane.showMessageDialog(null, "Same Node", "Warning!", JOptionPane.WARNING_MESSAGE);
                            removeEdge(e);
                    }
                    //else {
                     //   JOptionPane.showMessageDialog(null, "different Node");
                    //}
...
if(enable1) {
    ...
    public void removeEdge(IEdge... edgesToRemove)
    {
        ...
        for (IEdge anEdgeToRemove : edgesToRemove)
        {
            ...
            //remove edges for custom list
            if(anEdgeToRemove.getStartNode()==anEdgeToRemove.getEndNode()){
                alNodes.remove(anEdgeToRemove.getStartNode());
                //System.out.println(Collections.frequency(alNodes,anEdgeToRemove.getStartNode()));
            }
